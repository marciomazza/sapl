...
def get_materia_aberta(pk):
    return OrdemDia.objects.filter(
        sessao_plenaria_id=pk).filter(Q(votacao_aberta=True) | Q(discurssao_aberta=True)).last()
