...
def verifica_votacoes_abertas(request, pk):
    votacoes_abertas = SessaoPlenaria.objects.filter(
        (Q(ordemdia__votacao_aberta=True) | Q(ordemdia__discurssao_aberta=True) & ~Q(ordemdia__id=int(pk))) |
        (Q(expedientemateria__votacao_aberta=True) | Q(expedientemateria__discurssao_aberta=True) & ~Q(expedientemateria__id=int(pk)))).distinct()
...
def abrir_votacao(request, pk, spk):
    ...
    if (verifica_presenca(request, presenca_model, spk, is_leitura) and
        ...
        if  'discutir' in request.GET:
            materia_votacao.discurssao_aberta = True
        else:
            if materia_votacao.discurssao_aberta == True:
                materia_votacao.discurssao_aberta = False
